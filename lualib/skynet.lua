...
---@type fun(f:function) | fun(func:function, ti:any)
...
--- 让框架在ti个单位时间后，调用func这个函数，非阻塞
function skynet.timeout(ti, func)
	--- 创建一个timeout，获得对应session
...
--- 将当前coroutine挂起ti个单位时间，一个单位是1/100秒
...
--- 返回skynet节点进程内部时间，非真实时间
skynet.now = c.now
skynet.hpc = c.hpc	-- high performance counter

local traceid = 0
--- 在一个消息处理流程中，如果调用了这个 api ，将开启消息跟踪日志。
--- 每次调用都会生成一个唯一 tag ，所有当前执行流，和调用到的其它服务，都会计入日志中。
--- 具体解释，可以参考 https://blog.codingnow.com/2018/05/skynet_trace.html
...
skynet.redirect 不会调用 pack ，所以这里的 ... 必须是一个编码过的字符串，
或是 userdata 加一个长度。
]]
--- 它和 skynet.send 功能类似，但更细节一些
skynet.redirect = function(dest,source,typename,...)
	return c.redirect(dest, source, proto[typename].id, ...)
end
---@alias lightuserdata userdata
--- 可以将一组 lua 对象序列化为一个由 malloc 分配出来的 C 指针加一个数字长度
---@type fun(...):  lightuserdata, number
skynet.pack = assert(c.pack)
--- 和 skynet.pack 不同，它返回一个 lua string
skynet.packstring = assert(c.packstring)
---@type fun(message:any, size:any)
...
--[[
在内部生成一个唯一 session ，并向 address 提起请求
并阻塞等待对 session 的回应（可以不由 address 回应）
...
这个 API 被设计成传递一个 C 指针和长度，
而不是经过当前消息的 pack 函数打包。
或者你也可以省略 size 而传入一个字符串。
]]
...
--[[有时候，你需要挂起一个请求，等将来时机满足，再回应它。而回应的时候已经在别的 coroutine 中了]]
...
---@see raw_dispatch_message
--- 注册特定类消息的处理函数
---@param typename string
---@param func c.dispatch_func
