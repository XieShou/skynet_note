...
init_cb(struct snlua *l, struct skynet_context *ctx, const char * args, size_t sz) {
	...
	// 将全局域中的协程函数换成了init_profile中注册的resume/wrap
	...
	lua_pushlstring(L, args, sz);//push参数，加载的lua服务名称，例如bootstrap
	//拿上面整个文件调用pcall(loader(args))
	//int lua_pcall (lua_State *L, int nargs, int nresults, int msgh);
	//参数个数，返回值个数，错误处理函数在栈中的索引（0代表无）
	r = lua_pcall(L,1,0,1);
	if (r != LUA_OK) {
...
launch_cb(struct skynet_context * context, void *ud, int type, int session, uint32_t source , const void * msg, size_t sz) {
	...
	struct snlua *l = ud;
	skynet_callback(context, NULL, NULL);//先关掉自己的callback，为了在lua中安装cb
	int err = init_cb(l, context, msg, sz);//运行
	if (err) {
...
snlua_init(struct snlua *l, struct skynet_context *ctx, const char * args) {
	...
	memcpy(tmp, args, sz);
	skynet_callback(ctx, l , launch_cb);//安装cb
	const char * self = skynet_command(ctx, "REG", NULL);
	uint32_t handle_id = strtoul(self+1, NULL, 16);
	// it must be first message
	// 关键这一层目前还不清楚有没有走Lua层，如果没有的话就相当于测试下通讯然后调用上面安装的cb
	// 第一条消息将args参数中的内容发送给消息自己
	// 由上面注册的cb回调
